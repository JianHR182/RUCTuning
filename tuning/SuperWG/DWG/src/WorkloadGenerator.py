    ...
    def generate(self, condition):
        ...
        if rw_choice == 0:
            ...
            if tb_num == 1:
                ...
                if qcn == 0:
                    ...
                    if qcn == on or on == 0:
                        ...
                        not_aggregation_cols = from_tb_get_N_columns_for_aggregation(tbs=[tb_choice],
                                                                                     aggregationNum=qcn - on)
            ...
            # 整体思路上一阶段相似
                ...
                bool, tbs, tbs_sub_distribution = getNTable(dbs=dbs, n=2,
                                                            pdg=np.full(len(dbs.tables), 1.0 / len(dbs.tables)))
                tbs_ = list(tbs)
                if bool == -1:
                    ...
                    if qc_num > 1:
                        ...
                        for i in range(qc_num - 1):
                            ...
                            if col_choice.data_type in ['varchar', 'bpchar', 'blob']:
                                ...
                                if col_choice.data_type in ['varchar', 'bpchar']:
                                    data = rand_str_sampling2(min_len=col_choice.domain[0],
                                                              max_len=col_choice.domain[1])
                                    self.last_sql.add(key(value="\'" + data + "\'", type="value"))
                                else:
                                    self.last_sql.add(key(value="NULL", type="value"))
                            else:
                                ...
                                if col_choice.data_type == "timestamp":
                                    ...
                                    num_domain = int(col_choice.domain[1]) - int(col_choice.domain[0]) + 1
                                    if col_choice.data_type == "numeric" and col_choice.data_mod[0] == \
                                            col_choice.data_mod[1]:
                                        data = rand_decimal_sampling(col_choice.data_mod[0])
                                    else:
                                        data = rand_num_sampling(
                                            int(col_choice.domain[0]),
        ...
        # 写语句
            ...
            if insert_delete_choice == 0:
                ...
                for i in range(len(tb_choice.col)):
                    ...
                    if tb_choice.col[i].data_type in ['varchar', 'bpchar']:
                        ...
                        tmp += "\'" + rand_str_sampling2(min_len=tb_choice.col[i].domain[0],
                                                         max_len=tb_choice.col[i].domain[1]) + "\'"
                        ...
                        if tb_choice.col[i].data_type == "numeric" and tb_choice.col[i].data_mod[0] == \
                                tb_choice.col[i].data_mod[1]:
            ...
            # 删除语句，思路同理
                ...
                while col.data_type == 'blob':
                    tb_choice = self.dbs.tables[
                        rand_num_sampling(0, len(self.dbs.tables) - 1, condition.tableDistribution)]
                    res = from_tb_get_N_columns_for_aggregation([tb_choice], 1)
                    col = res[0]
                else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config_file', type=str, default='../../../' + config.json_extract_result_path)
